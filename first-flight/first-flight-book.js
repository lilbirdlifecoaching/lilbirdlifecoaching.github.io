(function () {
  ...
  async function startCheckout(email, buyerName) {
    ...
    try {
      ...
      const res = await fetch(WORKER_URL + '/create-first-flight-checkout-session', {
        method: 'POST',
      ...
      console.error('First Flight checkout:', err);
  ...
  function init() {
    ...
    if (urlPromo && promoInput) {
      ...
      // Suggest IMREADY by default for the offer path
      promoInput.value = 'IMREADY';
      applyPromoFromInput();
    }
  }
